        ...
        <div class="login-box">
            ...
            <el-form :model="loginForm" :rules="loginRules" ref="loginRef">
                ...
                <el-form-item prop="password">
                    <el-input v-model="loginForm.password" type="password" placeholder="请输入密码" maxlength="15"
    ...
    methods: {
        login() {
            this.$refs.loginRef.validate(async (valid) => {
                if (!valid) return
                try {
                    const res = await AUTH.UserLogin(this.loginForm)
                    console.log('用户登录请求成功', res)
                    if (res.data.code !== 0) return this.$message.error(res.data.message)
                    this.$store.commit('user/userToken', res.data.token)
                    this.$router.push('/')
                } catch (err) {
                    console.log('用户登录请求失败', err)
                }
            })
        }
    }
